...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{VERSION_2_BSFT}'):
        os.makedirs(f'{PROMPT_PATH}/{VERSION_2_BSFT}')
    
    with open(f'{PROMPT_PATH}/{VERSION_2_BSFT}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_2_BSFT}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_2_BSFT}/test.json', 'w') as f:
